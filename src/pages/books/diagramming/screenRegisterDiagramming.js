...
export default function ScreenRegisterDiagramming() {
    ...
    async function validatingData(){
        try {
            ...
            const ratedData = await IdentifyingDuplicate('diagramadores', ConcatenatingNames) 
            ...
            console.error('Error ocorrido na validação dos dados em validatingData - ScreenRegisterDiagramming ' + error)
        };        
    };
    
    const HandleSubmit = async () => await InsertDiagramming(dataFormatHoje, firstName, middleName,lastName, paisDiagramming, graduacaoDiagramming, numCPF, sexoDiagramming, racaDiagramming) ? setOpen(true) || setAlertSnack('success') ||  setTextSnackbar('Dados Inseridos com Sucesso') || setDisableButton(true) : setOpen(true) || setAlertSnack('error') ||  setTextSnackbar('Dados Não Forma Inseridos - Verificar Console'); 
